...
void Model::init_cl_data() {
  ...
  for (Mesh &mesh : meshes) {
    ...
    for (size_t i = 3; i < ind.size() - 3; i += 3) {
      if (ind[i] != ind[i - 3] && ind[i] != ind[i - 2] && ind[i] != ind[i - 1])
        if (ind[i + 1] != ind[i - 3] && ind[i + 1] != ind[i - 2] &&
            ind[i + 1] != ind[i - 1])
          if (ind[i + 2] != ind[i - 3] && ind[i + 2] != ind[i - 2] &&
              ind[i + 2] != ind[i - 1])
            nIndMeshes++;
    }
    ...
    for (Vertex &ver : vertices) {
      cl_vertices.emplace_back(
          cl_float3{{ver.Position.x, ver.Position.y, ver.Position.z}});
      if (ver.Position.x > max_pos.x)
        max_pos.x = ver.Position.x;
      if (ver.Position.x < min_pos.x)
        min_pos.x = ver.Position.x;
      if (ver.Position.y > max_pos.y)
        max_pos.y = ver.Position.y;
      if (ver.Position.y < min_pos.y)
        min_pos.y = ver.Position.y;
      if (ver.Position.z > max_pos.z)
        max_pos.z = ver.Position.z;
      if (ver.Position.z < min_pos.z)
        min_pos.z = ver.Position.z;
    }

    cl_meshes.emplace_back(
        cl_mesh{last_vertex, last_index, static_cast<cl_int>(vertices.size())});

    last_vertex += vertices.size();
    last_index += ind.size();
  }

  std::cout << "Max positions: " << max_pos.x << ", " << max_pos.y << ", "
            << max_pos.z << "\n";
  std::cout << "Min positions: " << min_pos.x << ", " << min_pos.y << ", "
            << min_pos.z << "\n";
