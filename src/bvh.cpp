...
void BVH::init() {   
  // Initialize buildData array for primitives
   ...
   recursiveBuild(root.get(), buildData, 0, primitives.size(), 
...
uint32_t BVH::flattenBVHTree(const BVHBuildNode *node, uint32_t *offset) {
  ...
  if (node->nPrimitives > 0) {
    ...
    linearNode->secondChildOffset = flattenBVHTree(node->getChild1(),
    offset);        
  }
  return myOffset;  
}
...
void BVH::recursiveBuild( BVHBuildNode *node, 
  ...
  if (nPrimitives < 15) {        
    createLeaf(node, buildData, start, end,
               orderedPrims, bbox, nPrimitives);    
  } else {    
    ...
    if (centroidBounds.pMax[dim] == centroidBounds.pMin[dim]) {
      createLeaf(node, buildData, start, end, orderedPrims,
                 bbox, nPrimitives);
      return;
    }
    ...
    if (nPrimitives==2) {
      ...
      BVHPrimitiveInfo *midPtr = std::partition(&buildData[start], &buildData[end-1]+1, 
        CompareToMid(dim, pmid));
      mid = midPtr - &buildData[0];
    } 
    node->InitNode();
    recursiveBuild(node->getChild0(), buildData, start, mid, totalNodes, orderedPrims);
    recursiveBuild(node->getChild1(), buildData, mid, end, totalNodes, orderedPrims);
