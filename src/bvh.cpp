...
void BVH::init() {
  ...
  std::cout << "Size of each BVH node:" << sizeof(BVH::LinearBVHNode)
            << " bytes \n";
  std::cout << "Size of BVH structure:"
            << totalNodes * sizeof(BVH::LinearBVHNode) / 1024 << " KB\n";
  std::cout << "Size of primitives:"
            << primitives.size() * sizeof(Triangle) / 1024 << " KB\n";
...
uint32_t BVH::flattenBVHTree(const BVHBuildNode *node, uint32_t *offset,
  ...
  if (node->nPrimitives > 0) {
    ...
    linearNode->secondChildOffset =
        flattenBVHTree(node->getChild1(), offset, maxlevel, level + 1);
  }
  return myOffset;
}
